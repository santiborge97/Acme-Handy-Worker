...
public interface ApplicationRepository extends JpaRepository<Application, Integer> {
	...
	@Query("select a from Application a where a.status='ACCEPTED' and a.fixUpTask.id = ?1")
	Application findApplicationByFixUpTaskId(int fixUpTaskId);

	@Query("select a from HandyWorker hw join hw.applications a where a.status='ACCEPTED' and hw.id = ?1")
	Collection<Application> findAcceptedApplications(int handyWorkerId);

	@Query("select count(a) from Application a where a.status='ACCEPTED' and a.fixUpTask.id = ?1")
	int countApplicationAcceptedByFixUpTask(int fixUpTaskId);
}
