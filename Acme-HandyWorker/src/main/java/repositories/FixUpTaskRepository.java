...
public interface FixUpTaskRepository extends JpaRepository<FixUpTask, Integer> {
	...
	int countFixUpTaskWithTicker(String ticker);

	@Query("select f from HandyWorker hw join hw.applications a join a.fixUpTask f where a.status='ACCEPTED' and hw.id = ?1")
	Collection<FixUpTask> findAcceptedFixUpTasks(int handyWorkerId);
}
