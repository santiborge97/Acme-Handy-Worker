...
public class PhaseService {
	...
	public Phase save(final Phase phase) {
		...
		Assert.isTrue(phaseStart.before(phaseEnd) || phaseStart.compareTo(phaseEnd) == 0);
		Assert.isTrue(taskStart.before(phaseStart) || taskStart.compareTo(phaseStart) == 0);
		Assert.isTrue(phaseStart.before(taskEnd) || phaseStart.compareTo(taskEnd) == 0);
		Assert.isTrue(taskStart.before(phaseEnd) || taskStart.compareTo(phaseEnd) == 0);
		Assert.isTrue(phaseEnd.before(taskEnd) || phaseEnd.compareTo(taskEnd) == 0);
	...
	public Boolean phaseHandyWorkerSecurity(final int fixUpTaskId) {
		Boolean res = false;

		final FixUpTask fixUpTask = this.fixUpTaskService.findOne(fixUpTaskId);

		final Application application = this.applicationService.findApplicationByFixUpTaskId(fixUpTask.getId());

		final HandyWorker owner = this.handyWorkerService.findHandyWorkerByApplicationId(application.getId());
